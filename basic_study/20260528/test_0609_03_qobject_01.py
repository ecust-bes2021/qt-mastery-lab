...
class ParentChildDemo(QWidget):
    ...
    def __init__(self):
        ...
        self._label_a = QLabel("标签a",parent=self,objectName="label_a")
        self._label_b = QLabel("标签b",parent=self, objectName="label_b")
        self._btn_tree = QPushButton("查看对象树",parent=self,objectName="btn_tree")

        # TODO(3): 创建一个没有 parent 的 QObject，objectName 设为 "orphan"
        #   思考：这个对象会出现在 self.children() 里吗？
        self._orphan = QObject(objectName="orphan")
        ...
        self._btn_tree.clicked.connect(self._print_tree)
    ...
    def _find_children_demo(self):
        ...
        if children:
            for child in children:
                print(f"Find child: {child.objectName()},Content: {child.text()}")
