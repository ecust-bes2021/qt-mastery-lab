...
class ModelViewBasicDemo(QWidget):
    ...
    def __init__(self):
        ...
        self.model = QStringListModel(["苹果","香蕉","橘子","葡萄","西瓜"])
        ...
        self._lineEdit.setPlaceholderText("输入新水果名称")

        # TODO(4): 创建按钮 "添加到末尾"
        #   点击时调用 self._add_item
        self._btn_add_item = QPushButton("添加到末尾")
        self._btn_add_item.clicked.connect(self._add_item)

        # TODO(5): 创建按钮 "删除选中项"
        #   点击时调用 self._remove_selected
        self._btn_romve_selected = QPushButton("删除选中项")
        self._btn_romve_selected.clicked.connect(self._remove_selected)

        # TODO(6): 创建按钮 "修改选中项"
        #   点击时调用 self._edit_selected
        self._btn_edit_selected = QPushButton("修改选中项")
        self._btn_edit_selected.clicked.connect(self._edit_selected)

        # TODO(7): 创建一个 QLabel 显示当前模型数据
        #   objectName 设为 "data_display"
        self._label_data_display = QLabel(objectName = "data_display")

        # TODO(8): 创建按钮 "打印模型数据"
        #   点击时调用 self._print_model_data
        #   把模型中所有数据打印到 data_display
        self._btn_print_model_data = QPushButton("打印模型数据")
        self._btn_print_model_data.clicked.connect(self._print_model_data)
        
        layout = QVBoxLayout()
        layout.addWidget(self.listView)
        layout.addWidget(self._lineEdit)
        layout.addWidget(self._btn_add_item)
        layout.addWidget(self._btn_romve_selected)
        layout.addWidget(self._btn_edit_selected)
        layout.addWidget(self._label_data_display)
        layout.addWidget(self._btn_print_model_data)
    ...
    def _add_item(self):
        ...
        txt = self._lineEdit.text().strip()
        row_count = self.model.rowCount()
        self.model.insertRow(row_count)
        if txt :
            self.model.setData(self.model.index(row_count), txt)
    ...
    def _edit_selected(self):
        ...
        if not current_index.isValid():
            return
        self.model.setData(current_index, txt)
    ...
    def _print_model_data(self):
        ...
        for i in range(self.model.rowCount()):
            items.append(self.model.data(self.model.index(i),Qt.DisplayRole))
        self._label_data_display.setText(",".join(items))
