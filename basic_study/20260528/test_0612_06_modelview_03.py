...
class CustomModelDemo(QWidget):
    def __init__(self):
        ...
        self.sensorDataModel = SensorDataModel()
        ...
        self.tableView.setItemDelegateForColumn(3,StatusDelegate(self))
        self.tableView.setModel(self.sensorDataModel)
        # TODO(5): 创建按钮 "添加传感器"
        #   点击时调用 self._add_sensor
        self._btn_add_sensor = QPushButton("添加传感器")
        ...
        #   观察：视图是否自动刷新？
        self._btn_update_values = QPushButton("模拟数据更新")
        self._btn_update_values.clicked.connect(self._update_values)

        # TODO(7): 创建一个 QLabel 显示说明
        #   文本："双击数值或状态列可以编辑"
        self._label = QLabel("双击数值或状态列可以编辑")

        # TODO(8): 布局
        layout = QVBoxLayout()
        layout.addWidget(self.tableView)
        layout.addWidget(self._btn_add_sensor)
        layout.addWidget(self._btn_update_values)
    ...
    def _add_sensor(self):
        ...
        model = self.sensorDataModel
        row_count = model.rowCount()
        model.beginInsertRows(QModelIndex(),row_count,row_count)
        model._data.append({"name":"新传感器","value":0.0,"unit":"?","status":"正常"})
        model.endInsertRows()
    ...
    def _update_values(self):
        ...
        import random
        for row in range(self.sensorDataModel.rowCount()):
            self.sensorDataModel._data[row]["value"] = round(random.uniform(0,100),1)
        top_left = self.sensorDataModel.index(0,1)
        bottom_right = self.sensorDataModel.index(self.sensorDataModel.rowCount()-1,1)
        self.sensorDataModel.dataChanged.emit(top_left,bottom_right,[Qt.DisplayRole])
