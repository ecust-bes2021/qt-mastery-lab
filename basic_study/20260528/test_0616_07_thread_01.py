...
class ThreadDemo(QWidget):
    ...
    def __init__(self):
        ...
        #   一个 QPushButton "开始任务"
        #   一个 QPushButton "重置"
        self._label_status = QLabel("空闲")
        self._textEdit = QTextEdit()
        self._textEdit.setReadOnly(True)
        self._btn_begin_task = QPushButton("开始任务")
        self._btn_reset = QPushButton("重置")
        self._btn_begin_task.clicked.connect(self._start_task)
        ...
        layout.addWidget(self._btn_begin_task)
        ...
        self._thread.started.connect(self._worker._work)
    ...
    def _start_task(self):
        ...
        if not self._thread.isRunning():
            self._thread.start()
            # ↓ 在这里调用 invokeMethod，替代 started 信号的作用
            # QMetaObject.invokeMethod(self._worker, "_work", Qt.ConnectionType.QueuedConnection)
            self._label_status.setText("运行中...")
            self._btn_begin_task.setEnabled(False)

    def _on_progress(self, value):
        # TODO(6): 更新进度显示
        #   更新状态标签显示百分比
        self._label_status.setText(f"进度为：{value}")
    ...
    def _on_finished(self):
        ...
        #   更新状态标签为 "完成"
        #   重新启用开始按钮
        self._label_status.setText("完成")
        self._btn_begin_task.setEnabled(True)
